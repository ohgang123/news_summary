...
def mistral_translate_text(english_title, english_summary, target_language='ko'):
    ...
    prompt = f"""
    Translate the following news article title and summary into fluent Korean. Ensure that professional terms, names, and expressions are translated accurately.
...
# NewsAPI를 통해 번역된 키워드로 뉴스 검색 (qInTitle 사용)
def get_news_by_keyword(keyword, from_date, to_date):
    ...
    url = (
        ...
        f"&sortBy=relevancy&language=en&pageSize=5&apiKey={NEWS_API_KEY}"  # 영문 기사 / 관련성 기준 정렬 
    )
    
    print(f"Request URL: {url}")  # URL 디버깅을 위해 출력
    ...
    if response.status_code == 200:
        ...
        if not articles:
            print(f"No articles found for keyword: {translated_keyword} from {from_date} to {to_date}")
        return articles
    else:
...
def summarize_news_articles(articles, target_language):
    ...
    for article in articles:
        ...
        # Mistral 모델로 요약 번역
        try:
            title_part, translated_summary = mistral_translate_text(title, summary, target_language)
        except Exception as e:
