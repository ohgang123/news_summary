...
if st.button('뉴스 검색'):
    if keyword:
        if start_date and end_date:
            from_date = news.format_date(start_date)
            to_date = news.format_date(end_date)
            
            # 한국어 또는 영어로 입력된 키워드를 처리하여 뉴스 검색
            articles = news.get_news_by_keyword(keyword, from_date, to_date)
            
            # Watsonx AI 모델을 통해 뉴스 기사 요약
            summarized_articles = news.summarize_news_articles(articles, target_language)
